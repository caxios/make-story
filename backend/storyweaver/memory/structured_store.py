...
def _character_filename(character_id: str) -> str:
    """One file per character, named so that two characters can never share it.

    The scheme lives in `storage.safe_filename` because every subject the story
    stores by an author-supplied id needs it, not only characters.
    """
    return safe_filename(CHARACTER_PREFIX, character_id)
